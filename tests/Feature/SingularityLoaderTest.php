...
test('the page draw loader component exists with its self-drawing mechanics', function () {
    ...
    expect($component)->not->toBeFalse()
        // Dependency-free animation: no gsap import (CSS + Web Animations API)...
        ->and($component)->not->toContain("from 'gsap'")
        // It measures the real DOM layout...
        ->and($component)->toContain('getBoundingClientRect')
        // It draws SVG strokes around page regions (WAAPI, not GSAP)...
        ->and($component)->toContain('strokeDashoffset')
        ->and($component)->toContain('rect.animate')
        // It includes a top progress bar driven by CSS scaleX + transitions...
        ->and($component)->toContain('page-loader-bar-inner');
});
