...
Each middleware registered in `bootstrap/app.php` needs a matching `tests/Feature/Http/Middleware/<Name>Test.php` (all middleware tests live under `tests/Feature/Http/Middleware/`, not flat in `tests/Feature/`); each Form Request in `app/Http/Requests/Dashboard/` needs a `tests/Unit/Http/Requests/<Name>Test.php`. Consent-gated shared props (HandleInertiaRequests adsense keys) must be tested for both accepted and non-accepted paths. Cover upload/remove endpoints (post + guide) must have upload, delete, and cascade-cleanup tests.

## Pure lib/ utilities and cookie/state composables must have vitest coverage
Pure functions in `resources/js/lib/` (e.g. `utils.ts`: `cn`, `toUrl`, `formatDate`) and composables whose logic is testable without a full Inertia stack (cookie parse/store/clear in `useConsent`, image processing in `useImageProcessor`) must have `*.test.ts` files alongside them using vitest + happy-dom. Follow the existing `useImageProcessor.test.ts` style: `describe`/`it`/`expect`/`vi` imports, `beforeEach` cleanup, mocked globals for side-effects (`window.location.reload`, `gtag`, `createImageBitmap`).
