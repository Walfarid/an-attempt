...
- The frontend types in `resources/js/data/portfolio.ts` (from the CV data) are the source of truth for portfolio entity shapes; backend tables mirror them. Currently: `profiles` (singleton), `skills`, `experiences`, `projects` (+ `project_screenshots` + `project_skill` pivot), `posts`. Education + publications exist in the frontend data but have no tables yet — add when the dashboard needs them.
- Contact intake (`POST /contact`, public): per-IP `throttle:5,1` middleware, honeypot field `website` (rule `prohibited`), Cloudflare Turnstile token verified server-side via `App\Rules\Turnstile` against siteverify — the check is skipped only while `TURNSTILE_SECRET_KEY` is empty (local dev); production must set it. Notification mail is queued (`ContactMessageReceived`, plain-text view, reply-to = sender) to `config('contact.notification_email')`; `read_at` marks triage.
