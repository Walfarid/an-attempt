<!--
    HeroIllustration.vue — hero artwork for the home page.

    Source: "Programming" illustration from unDraw (undraw.co), licensed for
    free commercial/personal use, no attribution required. Recolored so every
    fill resolves through the --ill-* tokens below: light mode mirrors the
    site tokens (viridian accent, ink, rule, paper), dark mode inverts them
    so the artwork reads as ink-on-dark like the rest of the theme.

    Per the unDraw license this asset may not be redistributed as a pack,
    used to train AI models, or otherwise reproduced wholesale — do not
    extract it into standalone downloads.

    The .c-* wrappers keep the existing useHeroScene entrance alive: the
    dashed rings trace in, the small nodes pop — the illustration itself is
    static art.

    Decorative illustration only — the parent wrapper carries aria-hidden.
-->
<template>
    <svg
    ...
    >
        <!-- Head (kept ink/white in both themes so the figure never merges
             with the chair backrest) -->
        <path
            d="M200.584,0h430.7a20.7,20.7,0,0,1,0,41.4h-430.7a20.7,20.7,0,0,1,0-41.4Z"
            transform="translate(28.583 435)"
            fill="var(--ill-head)"
        />
        <!-- Chair backrest, shifted darker than the head -->
        <path
            d="M0,2.7H138.6V0H0Z"
            transform="translate(28.583 435)"
            fill="var(--ill-chair)"
        />
        <!-- Nape skin tone, positioned at the back of the neck -->
        <path
            d="M-60.3,396.1c85.7,0,150.3,21.9,195.8,61.1l-54.3,68.9c-4.9-118.3-79.8-102.9-141.5-102.9v-27.2Z"
            transform="translate(28.583 435)"
            fill="var(--ill-skin)"
        />
        ...
        <!-- Entrance accents: dashed rings + energy nodes (see useHeroScene) -->
        <g class="c-rings">
            <circle
                cx="756"
                cy="88"
...
.dark .hero-svg-shape {
    /* Preserve the light artwork's luminance order in dark mode (deepest
       shapes stay the lightest, panels stay darkest) so adjacent shapes
       never collapse into the same value — a naive inversion turned the
       figure and chair into one white blob. */
    --ill-accent: #5bb89a;
    --ill-mid: #6e6a63;
    --ill-skin: #8a8478;
    --ill-head: #d6d2cb;
